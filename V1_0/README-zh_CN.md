...
>
> **⚠️ Flash 寿命警告**
> - Flash 存储器有有限的擦写次数（通常为 1 万-10 万次）
> - 避免频繁使用 0x64 功能码以防 Flash 过早损坏
> - 实现中包含了数据对比机制以尽量减少不必要的写入
...
### Flash 操作优化
*   **数据对比**: 写入Flash前会先比较当前值，避免不必要的写入以延长Flash寿命
*   **兼容性**: 增强的Flash擦除/编程操作支持更多STM32系列(F1、F4、G0、G4、L4、L5、H7)
*   **性能说明**: Flash操作期间CPU会暂停执行。频繁使用0x64功能码会快速消耗Flash擦写寿命(通常为1万-10万次)

### 乒乓缓冲(双缓冲)机制
*   **解决的问题**: 竞态条件 - 当`Modbus_Process()`正在处理前一帧数据时，新数据覆盖`rx_buf`
*   **解决方案**: 双接收缓冲区(`rx_buf_a`和`rx_buf_b`)，在中断回调中自动切换
*   **工作原理**:
    - 中断接收数据到缓冲区A，设置就绪标志
    - 主循环处理缓冲区A时，中断同时接收数据到缓冲区B
    - 即使高频通信也不会丢失数据
*   **优势**: 消除连续 Modbus 请求时的缓冲区数据损坏问题

### 动态发送超时计算
*   **解决的问题**: 硬编码100ms超时导致低波特率时发送失败
*   **问题场景**: 在1200波特率下发送256字节需要约2.1秒，超过固定100ms超时
*   **解决方案**: 根据波特率和数据长度动态计算超时时间
*   **计算公式**: `timeout_ms = (字节数 * 10位 * 1000) / 波特率 + 安全余量`
*   **安全余量**: 50ms或10%传输时间(取较大值)，最小超时100ms
*   **优势**: 在所有支持的波特率(1200~115200)下都能可靠发送

### CRC16算法选择
*   **配置项**: 在`modbus_config.h`中设置`MODBUS_CRC_USE_TABLE`
*   **可选值**:
    - `0` (默认): **移位法** - 代码体积小，计算较慢
    - `1`: **查表法** - 速度快约10倍，需额外512字节ROM存储查找表
*   **选择建议**:
    - ROM空间紧张: 使用移位法 (0)
    - 高波特率(115200+)或通信频繁: 使用查表法 (1)
*   **性能对比** (STM32F103 @ 72MHz, 256字节):
    - 移位法: 约1.5ms
    - 查表法: 约0.15ms
