 ...
 * @note    支持两种算法:
 *          - 移位法: 代码体积小，计算较慢
 *          - 查表法: 速度快约10倍，需512字节ROM
 *          通过 MODBUS_CRC_USE_TABLE 宏切换
 */
#if MODBUS_CRC_USE_TABLE == 1

/* CRC16-Modbus查找表 (512字节) */
...
#else  /* 移位法 (MODBUS_CRC_USE_TABLE == 0) */
