...
// 全局变量定义
#if MB_COIL_COUNT > 0
uint8_t  mb_coils[(MB_COIL_COUNT + 7) / 8] = {0};
#endif

#if MB_DISCRETE_COUNT > 0
uint8_t  mb_discrete_inputs[(MB_DISCRETE_COUNT + 7) / 8] = {0};
#endif
...
void Modbus_Process(void) {
    ...
    switch (func_code) {
        ...
        case MB_FUNC_WRITE_MULTI_COILS: // 0x0F (15)
        {
            start_addr = (hmodbus.rx_buf[2] << 8) | hmodbus.rx_buf[3];
            quantity = (hmodbus.rx_buf[4] << 8) | hmodbus.rx_buf[5];
            byte_count = hmodbus.rx_buf[6]; // 协议中 byte count 在 index 6
        ...
        case MB_FUNC_WRITE_MULTI_REGS: // 0x10 (16)
        {
            start_addr = (hmodbus.rx_buf[2] << 8) | hmodbus.rx_buf[3];
            quantity = (hmodbus.rx_buf[4] << 8) | hmodbus.rx_buf[5];
